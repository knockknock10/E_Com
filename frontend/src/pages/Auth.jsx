...
const Auth = () => {
  ...
  const validate = () => {
    const newErrors = {};
    if (!email) newErrors.email = 'Email is required';else if (!/\S+@\S+\.\S+/.test(email)) newErrors.email = 'Email format is invalid';
  ...
  return <div className="min-h-screen bg-primary flex w-full">
      {/* Left Area - Branding / Image (Split Screen) */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-dark">
        <img src="https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80" alt="Luxury Architecture" className="absolute inset-0 w-full h-full object-cover opacity-60 mix-blend-overlay" />
          ...
          duration: 0.4
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <AnimatePresence mode="popLayout">
                ...
                }} className={`w-full px-5 py-4 bg-primary/50 border ${errors.name ? 'border-red-500 focus:border-red-500' : 'border-black/5 focus:border-accent'} rounded-xl focus:outline-none focus:bg-white transition-all text-dark shadow-sm`} placeholder="John Doe" />
              ...
              }} className={`w-full px-5 py-4 bg-primary/50 border ${errors.email ? 'border-red-500 focus:border-red-500' : 'border-black/5 focus:border-accent'} rounded-xl focus:outline-none focus:bg-white transition-all text-dark shadow-sm`} placeholder="you@example.com" />
              ...
              }} className={`w-full px-5 py-4 bg-primary/50 border ${errors.password ? 'border-red-500 focus:border-red-500' : 'border-black/5 focus:border-accent'} rounded-xl focus:outline-none focus:bg-white transition-all text-dark shadow-sm`} placeholder="••••••••" />
              ...
              <AnimatePresence mode="popLayout">
                ...
                }} className={`w-full px-5 py-4 bg-primary/50 border ${errors.confirmPassword ? 'border-red-500 focus:border-red-500' : 'border-black/5 focus:border-accent'} rounded-xl focus:outline-none focus:bg-white transition-all text-dark shadow-sm`} placeholder="••••••••" />
                    {errors.confirmPassword && <p className="text-red-500 text-xs mt-1.5 ml-1 font-medium">{errors.confirmPassword}</p>}
                  </motion.div>}
              </AnimatePresence>

              <button type="submit" disabled={loading} className="w-full mt-6 py-4 bg-dark text-white font-bold flex items-center justify-center gap-2 rounded-xl hover:bg-accent hover:shadow-lg hover:shadow-accent/20 transition-all active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100">
