const Contact = () => {
  return <section id="contact" className="py-24 lg:py-32 px-6 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 lg:p-16 shadow-2xl shadow-black/5 border border-black/5 flex flex-col lg:flex-row gap-12 lg:gap-16 relative z-10 w-full">
          ...
          {/* Contact Form */}
          <form onSubmit={e => e.preventDefault()} className="w-full lg:w-7/12 flex flex-col gap-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-dark">First Name</label>
                <input type="text" placeholder="John" className="w-full px-5 py-4 bg-primary border border-black/5 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-dark shadow-sm" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-dark">Last Name</label>
                <input type="text" placeholder="Doe" className="w-full px-5 py-4 bg-primary border border-black/5 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-dark shadow-sm" />
              </div>
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-dark">Email Address</label>
              <input type="email" placeholder="john@example.com" className="w-full px-5 py-4 bg-primary border border-black/5 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-dark shadow-sm" />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-dark">Message</label>
              <textarea rows={5} placeholder="Tell us about the property you are looking for..." className="w-full px-5 py-4 bg-primary border border-black/5 rounded-xl focus:outline-none focus:border-accent focus:bg-white transition-all text-dark resize-none hide-scrollbar shadow-sm" />
            </div>

            <button type="submit" className="mt-2 px-8 py-4 bg-dark text-white font-bold rounded-xl hover:bg-accent hover:shadow-xl hover:shadow-accent/20 transition-all active:scale-[0.98] w-full sm:w-auto self-start">
              Send Inquiry
            </button>
          </form>
